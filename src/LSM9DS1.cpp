...
extern HAL_GPIO LSM9DS1_CSAG;        //PB2
extern HAL_GPIO LSM9DS1_CSM;         //PC0
extern HAL_I2C  LSM9DS1_I2C;        //I2C1
extern HAL_SPI  LSM9DS1_SPI;        //SPI1
...
void LSM9DS1::xgWriteByte(uint8_t subAddress, uint8_t data)
{
	...
	if (settings.device.commInterface == IMU_MODE_I2C)
		...
		SPIwriteByte(&LSM9DS1_CSAG, subAddress, data);
}
...
void LSM9DS1::mWriteByte(uint8_t subAddress, uint8_t data)
{
	...
	if (settings.device.commInterface == IMU_MODE_I2C)
		...
		return SPIwriteByte(&LSM9DS1_CSM, subAddress, data);
}
...
uint8_t LSM9DS1::xgReadByte(uint8_t subAddress)
{
	...
	if (settings.device.commInterface == IMU_MODE_I2C)
		...
		return SPIreadByte(&LSM9DS1_CSAG, subAddress);
	return -1;
}
...
uint8_t LSM9DS1::xgReadBytes(uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	...
	if (settings.device.commInterface == IMU_MODE_I2C)
		...
		return SPIreadBytes(&LSM9DS1_CSAG, subAddress, dest, count);
	return -1;
}
...
uint8_t LSM9DS1::mReadByte(uint8_t subAddress)
{
	...
	if (settings.device.commInterface == IMU_MODE_I2C)
		...
		return SPIreadByte(&LSM9DS1_CSM, subAddress);
	return -1;
}
...
uint8_t LSM9DS1::mReadBytes(uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	...
	if (settings.device.commInterface == IMU_MODE_I2C)
		...
		return SPIreadBytes(&LSM9DS1_CSM, subAddress, dest, count);
	return -1;
}

void LSM9DS1::initSPI()
{
	LSM9DS1_SPI.init(1000000);
	LSM9DS1_CSAG.init(true,1,1);
	LSM9DS1_CSM.init(true,1,1);
}

void LSM9DS1::SPIwriteByte(HAL_GPIO* csPin, uint8_t subAddress, uint8_t data)
{
	// If write, bit 0 of the subAddress (MSB) should be 0
    // If single byte, bit 1 of the subAddress should be 0
	subAddress = subAddress & 0x3F;
	uint8_t DATA[2] = {subAddress, data};
	csPin->setPins(0); // Initiate communication
	LSM9DS1_SPI.write(DATA, 2); // Send data
	csPin->setPins(1); // Close communication
}

uint8_t LSM9DS1::SPIreadByte(HAL_GPIO* csPin, uint8_t subAddress)
{
	uint8_t temp;
...
uint8_t LSM9DS1::SPIreadBytes(HAL_GPIO* csPin, uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	// To indicate a read, set bit 0 (msb) of first byte to 1
	uint8_t rAddress = 0x80 | (subAddress & 0x3F);
	// Mag SPI port is different. If we're reading multiple bytes,
	// set bit 1 to 1. The remaining six bytes are the address to be read
	if ((csPin == &LSM9DS1_CSM) && count > 1)
		rAddress |= 0x40;

	uint8_t temp[count+1];
	csPin->setPins(0); // Initiate communication
	LSM9DS1_SPI.writeRead(&rAddress, 1, temp, count+1);
	csPin->setPins(1); // Close communication
...
{
	uint8_t DATA[2] = {subAddress, data};
	LSM9DS1_I2C.write(address, DATA, 2);
}

uint8_t LSM9DS1::I2CreadByte(uint8_t address, uint8_t subAddress)
{
	uint8_t DATA[1];
	LSM9DS1_I2C.writeRead(address, &subAddress, 1, DATA, 1);
	return DATA[0];
}
...
uint8_t LSM9DS1::I2CreadBytes(uint8_t address, uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	...
	LSM9DS1_I2C.writeRead(address, &subAddress, 1, dest, count);
